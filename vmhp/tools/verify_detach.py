...
from vmhp.tools.common import infer_feature_dims, iter_feature_batches, load_feature_file, num_classes_from_features, packet_from_batch
...
def main():
    ...
    if ckpt is not None:
        model.load_state_dict(ckpt.get("model", ckpt.get("plugin")), strict=False)
    model.eval()
    ...
    with torch.no_grad():
        for batch in iter_feature_batches(features, args.batch_size, shuffle=False, device=device):
            ...
            disabled, _ = model(packet, use_vmhp=False)
