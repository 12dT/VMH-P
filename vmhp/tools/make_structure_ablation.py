...
from vmhp.core.vertical_hierarchy import VerticalHyperbolicSentimentHierarchy
from vmhp.core.horizontal_relation_field import safe_row_l1_norm
...
def rebuild_tree_state(structure: dict, topology: dict):
    ...
    tree = VerticalHyperbolicSentimentHierarchy(
        ...
        gamma=float(cfg.get("edge_cost_weight", cfg.get("gamma", 0.1))),
    )
...
def main():
    parser = argparse.ArgumentParser(description="Create VMH-P structure variants for controlled ablations.")
    ...
    if args.variant == "vertical_only":
        ...
        structure = rebuild_tree_state(structure, topology)
        if args.variant == "horizontal_only":
            structure["cfg"]["horizontal_only"] = True
