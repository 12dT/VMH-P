...
from vmhp.tools.common import iter_feature_batches, load_feature_file, packet_from_batch, weighted_f1_acc
...
def load_vmhp(checkpoint_path: str, structure_path: str, device: torch.device):
    ...
    state = dict(checkpoint.get("model", checkpoint.get("plugin", {})))
    model.load_state_dict(state, strict=False)
    model.eval()
    return model


load_plugin = load_vmhp
...
def collect(plugin, features: Dict, batch_size: int, device: torch.device) -> Dict[str, torch.Tensor]:
    ...
    for batch in iter_feature_batches(features, batch_size, shuffle=False, device=device):
        ...
        rows["structured_logits"].append(aux["structured_logits"].detach().cpu())
        rows["plugin_logits"].append(aux["plugin_logits"].detach().cpu())
        rows["final_logits"].append(final_logits.detach().cpu())
        conflict = aux.get("conflict_gate", torch.zeros_like(aux["plugin_logits"]))
        rows["conflict"].append(conflict.detach().cpu())
...
def main():
    parser = argparse.ArgumentParser(description="Train a VMH-P post-hoc switch selector on cached features.")
    ...
    dev = collect(model, load_feature_file(args.dev_features), args.batch_size, device)
    test = collect(model, load_feature_file(args.test_features), args.batch_size, device)
