...
from vmhp.tools.rescue_search import collect, load_vmhp, metric
...
def parse_candidate(value: str) -> Tuple[str, str, str]:
    # name=/path/to/best_vmhp.pt:tree
...
def main():
    parser = argparse.ArgumentParser(description="Calibrate a pairwise VMH-P branch ensemble on cached features.")
    ...
    for raw in args.candidate:
        ...
        model = load_vmhp(checkpoint, args.structure, device)
        dev = collect(model, dev_features, args.batch_size, device)
        test = collect(model, test_features, args.batch_size, device)
    ...
    result = {
        "format": "vmhp_branch_ensemble_v1",
