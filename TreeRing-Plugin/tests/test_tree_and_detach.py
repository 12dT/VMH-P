...
def test_tree_output_and_detach_property():
    ...
    head = TreeRingHead({"text": 10, "vision": 12, "multimodal": 14}, 3, cfg)
...
def test_routing_probabilities_sum_to_one():
    cfg = TreeRingConfig(tree_dim=8, num_internal=2)
    head = TreeRingHead({"text": 10}, 3, cfg)
    head.initialize_from_class_centers(torch.randn(3, 8) * 0.1)
    z = head.ufa["text"](torch.randn(5, 10))
    x = head.geometry.exp_origin(z)
    routing = head.tree.compute_routing(x)["edge_log_prob"].exp()
    for parent_id in torch.unique(head.tree.edge_parent).tolist():
        edge_ids = torch.nonzero(head.tree.edge_parent == int(parent_id), as_tuple=False).view(-1)
