...
from tools.rescue_search import load_plugin
from train_plugin import evaluate
...
def _copy_relation(plugin, structure):
    pos = structure["relation_pos_init"].to(
        device=plugin.ring.A_pos_init.device,
        dtype=plugin.ring.A_pos_init.dtype,
    )
    neg = structure["relation_neg_init"].to(
        device=plugin.ring.A_neg_init.device,
        dtype=plugin.ring.A_neg_init.dtype,
    )
    with torch.no_grad():
        plugin.ring.A_pos_init.copy_(pos)
        plugin.ring.A_neg_init.copy_(neg)


def apply_counterfactual(plugin, mode, structure_path):
    if mode == "full":
        return None
    ...
    with torch.no_grad():
        if mode == "vertical_only":
            plugin.ring.A_pos_init.zero_()
            plugin.ring.A_neg_init.zero_()
            plugin.ring.theta_pos.zero_()
            plugin.ring.theta_neg.zero_()
        ...
        if mode == "random_relation":
            ...
            _copy_relation(plugin, structure)
        ...
        if mode == "random_tree":
            ...
            plugin.tree.set_topology(topology)
            edge_cost = plugin.tree._compute_initial_edge_cost(plugin.tree.node_tangent_params)
            plugin.tree.edge_cost.copy_(edge_cost)
            return {
                "changed": [
                    "class_to_internal",
                    "edge_parent",
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate structural counterfactuals with one frozen Full SET-P checkpoint."
    )
    ...
    plugin = load_plugin(args.checkpoint, None, device)
    applied = apply_counterfactual(plugin, args.mode, args.structure)
    plugin.eval()
    ...
    test_metrics = evaluate(plugin, test, args.batch_size, device)
    checkpoint = torch.load(args.checkpoint, map_location="cpu")
    result = {
        "format": "setp_frozen_full_structure_counterfactual_v1",
