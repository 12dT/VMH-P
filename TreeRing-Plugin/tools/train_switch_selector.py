...
from tools.rescue_search import collect, load_plugin, make_feature_bank, metric, score
...
def main():
    parser = argparse.ArgumentParser(description="Train a TreeRing switch selector from base to an auxiliary branch.")
    ...
    train_rows = collect(plugin, load_feature_file(args.train_features), args.collect_batch_size, device)
    dev_rows = collect(plugin, load_feature_file(args.dev_features), args.collect_batch_size, device)
    test_rows = collect(plugin, load_feature_file(args.test_features), args.collect_batch_size, device)
    ...
    result = {
        "format": "treering_switch_selector_v1",
