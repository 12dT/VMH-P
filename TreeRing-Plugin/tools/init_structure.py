...
from common import iter_feature_batches, load_feature_file
from core.plugin_head import TreeRingConfig, TreeRingHead
from topology.build_relations import build_relation_inits
from topology.build_tree import build_relation_aware_tree_topology, build_tree_topology
from topology.class_centers import compute_class_centers


@torch.no_grad()
def project_all_sources(plugin, features, batch_size, device):
    projected = {name: [] for name in plugin.ufa.keys()}
    labels = []
    plugin.eval()
    for batch in iter_feature_batches(features, batch_size, shuffle=False, device=device):
        ...
        for name in plugin.ufa:
            if batch.get(name) is not None:
                projected[name].append(plugin.ufa[name](batch[name]).detach().cpu())
...
def main():
    ...
    plugin = TreeRingHead(warm["feature_dims"], warm["num_classes"], cfg).to(device)
    plugin.ufa.load_state_dict(warm["ufa"], strict=True)

    projected, labels = project_all_sources(plugin, train, args.batch_size, device)
    ...
    plugin.initialize_from_class_centers(class_centers.to(device), topology=topology)
    ...
    print("saved global TreeRing structure to", args.output_dir)
