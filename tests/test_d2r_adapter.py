...
from vmhp.adapters.d2r_adapter import D2RAdapter
...
def test_d2r_detached_vmhp_returns_base_logits_and_core_has_no_d2r_branch():
    packet = D2RAdapter(DummyD2R(), detach_backbone=True).extract(make_batch())
    cfg = VMHPConfig(tree_dim=8, num_internal=2)
    head = VMHPHead({"text": 4, "vision": 5, "multimodal": 6}, 3, cfg)
